...
Wu = np.random.randn(hidden_size, vector_len)*0.01
W = np.random.randn(hidden_size, vector_len)*0.01
Ur = np.random.randn(hidden_size, hidden_size)*0.01
Uu = np.random.randn(hidden_size, hidden_size)*0.01
...
def lossFun(phrase, target, hprev):
  ...
  for t in range(len(phrase)):
    ...
    #GRU Implementation
    rt = sigmoid(np.dot(Wr,xs[t]) + np.dot(Ur,hs[t-1]) + br)
    ut = sigmoid(np.dot(Wu,xs[t]) + np.dot(Uu,hs[t-1]) + bu)
    ct = np.tanh(np.dot(W,xs[t]) + np.dot(U,rt*hs[t-1]) + b)
    hs[t] = ut*hs[t-1] + (1-ut)*ct # hidden state
  ...
  dWr, dWu, dW, dWhy = np.zeros_like(Wr), np.zeros_like(Wu), np.zeros_like(W), np.zeros_like(Why)
  dUr, dUu, dU = np.zeros_like(Ur), np.zeros_like(Uu), np.zeros_like(U)
  dbr, dbu, db, dby = np.zeros_like(br), np.zeros_like(bu), np.zeros_like(b), np.zeros_like(by)
